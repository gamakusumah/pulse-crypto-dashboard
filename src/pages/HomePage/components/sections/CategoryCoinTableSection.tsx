...
import { CategoryTabs, CATEGORY_ALL_VALUE } from '@/pages/HomePage/components/CategoryTabs';
import { CoinTable } from '@/pages/HomePage/components/CoinTable';
import { useCategories, useMarkets } from '@/pages/HomePage/hooks';
...
export function CategoryCoinTableSection() {
  ...
  return (
    <section aria-labelledby="markets-heading" className="space-y-3">
      <SectionTitle title={t('home.categoryMarket.title')} description={t('home.categoryMarket.description')} />

      {categoriesQuery.isPending ? (
        <LoadingSkeleton variant="text" className="h-8 w-full" />
      ) : categoriesQuery.isError ? null : categoriesQuery.data ? (
        <CategoryTabs categories={categoriesQuery.data} value={category} onValueChange={handleCategoryChange} />
      ) : null}
      ...
      {marketsQuery.isPending ? (
        ...
        <CoinTable
          ...
          page={page}
          hasNextPage={page < PAGINATION.MARKETS_MAX_PAGE && marketsQuery.data.length === PAGINATION.MARKETS_PER_PAGE}
