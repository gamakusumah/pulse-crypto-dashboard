...
import {
  ...
  fetchNews,
  fetchTrending,
} from '@/pages/HomePage/api/home.api';
import {
  ...
  mapMarketCoin,
  mapTrendingCoin,
} from '@/pages/HomePage/api/home.mapper';
...
export function trendingQueryOptions() {
  return queryOptions({
    ...
    queryFn: async () => {
      ...
      return response.coins.slice(0, HOME_PAGE_LIMITS.TRENDING_COINS).map((entry) => mapTrendingCoin(entry.item));
    },
  });
}
